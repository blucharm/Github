...
opkg install luci luci-ssl uhttpd luci-app-openvpn \
    ...
    curl ca-certificates dnsmasq-full nftables-full || true
...
# ==================== 4.1. ENHANCED DNSMASQ CONFIG (like AntidetectRouter) ====================
echo "==> [4.1/10] Enhanced dnsmasq config for VPN DNS relay..."
...
# Set rp_filter = 0 for all/tunX to prevent 'local delivery' drop
for i in all default tun0 tun1; do sysctl -w net.ipv4.conf.$i.rp_filter=0 2>/dev/null || true; done
...
######## New: Allow DNS from VPN clients to router (UDP/TCP 53), as in @vektort13 #########
for proto in udp tcp; do
  uci add firewall rule
  uci set firewall.@rule[-1].name="allow-dns-from-vpn-${proto}"
  uci set firewall.@rule[-1].src='vpnserver'
...
echo "==> [8/10] Creating custom nft NAT rules..."
cat > /etc/firewall.user <<'FWUSER'
#!/bin/sh
# After fw4 runs, ensure VPN NAT is idempotent using nftables, inspired by AntidetectRouter.sh
sleep 3
...
if command -v nft >/dev/null 2>&1; then
    # Remove duplicate rules first (safer)
    nft --handle list chain inet fw4 srcnat 2>/dev/null | grep "$OVPN_NET" | \
      awk -F'# handle ' '{print $2}' | while read -r h; do nft delete rule inet fw4 srcnat handle $h 2>/dev/null; done
    # NAT for client subnet
    nft add rule inet fw4 srcnat oifname "$WAN_IF" ip saddr "$OVPN_NET" counter masquerade 2>/dev/null || true
fi

logger "Enhanced OpenVPN nft NAT rules applied"
FWUSER

chmod +x /etc/firewall.user
/etc/init.d/firewall restart || true

# ==================== 9. DNS CONFIG (preserved options) ====================
echo "==> [9/10] Configuring system DNS…"
...
# (dnsmasq config is already handled above)
...
echo ""
echo "Setup finished at $(date). Please test VPN client internet access or DNS. If still not working,"
echo "run: nft list ruleset | sed -n '1,240p' and paste output here for analysis."
echo ""
